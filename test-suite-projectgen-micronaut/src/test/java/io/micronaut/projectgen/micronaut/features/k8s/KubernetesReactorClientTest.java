...
class KubernetesReactorClientTest {
    @Test
    void kubernetesReactorClientFeaturesAddsTheDependency(ProjectGenerator micronautProjectGenerator) throws Exception {
        Options options = OptionsFixture.defaultGradle().features(List.of("kubernetes-reactor-client")).build();
        ...
        assertTrue(verifier.hasDependency("io.micronaut.reactor", "micronaut-reactor", Scope.COMPILE), buildGradle);
    }

    @Disabled //TODO enable this test
    @Test
    void kubernetesReactorClientFeaturesAddsTheDiscoveryCoreDependencyForGroovyAndMaven(ProjectGenerator micronautProjectGenerator) throws Exception {
        Options options = OptionsFixture.defaultGradle().buildTools(List.of(BuildTool.MAVEN))
            .features(List.of("kubernetes-reactor-client"))
    ...
    @Test
    void kubernetesReactorClientFeaturesAddsTheLinkInReadmeFile(ProjectGenerator micronautProjectGenerator) throws Exception {
        Options options = OptionsFixture.defaultGradle().features(List.of("kubernetes-reactor-client")).build();
        Map<String, String> project = generateProject(micronautProjectGenerator, options);
