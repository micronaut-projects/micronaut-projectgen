...
class OpenTelemetryZipkinTest {

    @Test
    void zipkinConfiguration(ProjectGenerator micronautProjectGenerator) throws Exception {
        Options options = OptionsFixture.defaultGradle().features(List.of("tracing-opentelemetry-zipkin")).build();
        Map<String, String> project = generateProject(micronautProjectGenerator, options);
        Properties applicationProperties = ConfigurationUtils.loadApplicationProperties(project);
        assertEquals("zipkin", applicationProperties.getProperty("otel.traces.exporter"));
    }
    ...
    void zipkinFeaturesAddsTheDependency(ProjectGenerator micronautProjectGenerator) throws Exception {
        ...
        assertTrue(verifier.hasDependency("io.opentelemetry", "opentelemetry-exporter-zipkin", Scope.COMPILE), buildGradle);
    }
