...
class DistributedConfigConsulTest {
    @Test
    void distributedConfigConsulFeaturesAddsTheDependency(ProjectGenerator micronautProjectGenerator) throws Exception {
        Options options = OptionsFixture.defaultGradle().features(List.of("config-consul")).build();
    ...
    @Test
    void distributedConfigConsulFeaturesAddsTheLinkInReadmeFile(ProjectGenerator micronautProjectGenerator) throws Exception {
        Options options = OptionsFixture.defaultGradle().features(List.of("config-consul")).build();
        Map<String, String> project = generateProject(micronautProjectGenerator, options);
    ...
    @Test
    void distributedConfigConsulFeaturesAddsBootstrapConfiguration(ProjectGenerator micronautProjectGenerator) throws Exception {
        String name = "foo";
        Options options = OptionsFixture.defaultGradle().name(name).features(List.of("config-consul")).build();
        Map<String, String> project = generateProject(micronautProjectGenerator, options);
