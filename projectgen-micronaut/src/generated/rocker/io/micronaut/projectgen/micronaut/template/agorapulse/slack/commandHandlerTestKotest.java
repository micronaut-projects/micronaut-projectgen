package io.micronaut.projectgen.micronaut.template.agorapulse.slack;

import java.io.IOException;
import com.fizzed.rocker.ForIterator;
import com.fizzed.rocker.RenderingException;
import com.fizzed.rocker.RockerContent;
import com.fizzed.rocker.RockerOutput;
import com.fizzed.rocker.runtime.DefaultRockerTemplate;
import com.fizzed.rocker.runtime.PlainTextUnloadedClassLoader;
// import @ [1:1]
import io.micronaut.projectgen.core.generator.Project;

/*
 * Auto generated code to render template io/micronaut/projectgen/micronaut/template/agorapulse/slack/commandHandlerTestKotest.rocker.raw
 * Do not edit this file. Changes will eventually be overwritten by Rocker parser!
 */
@SuppressWarnings("unused")
public class commandHandlerTestKotest extends com.fizzed.rocker.runtime.DefaultRockerModel {

    static public com.fizzed.rocker.ContentType getContentType() { return com.fizzed.rocker.ContentType.RAW; }
    static public String getTemplateName() { return "commandHandlerTestKotest.rocker.raw"; }
    static public String getTemplatePackageName() { return "io.micronaut.projectgen.micronaut.template.agorapulse.slack"; }
    static public String getHeaderHash() { return "-1654727492"; }
    static public String[] getArgumentNames() { return new String[] { "project" }; }

    // argument @ [3:2]
    private Project project;

    public commandHandlerTestKotest project(Project project) {
        this.project = project;
        return this;
    }

    public Project project() {
        return this.project;
    }

    static public commandHandlerTestKotest template(Project project) {
        return new commandHandlerTestKotest()
            .project(project);
    }

    @Override
    protected DefaultRockerTemplate buildTemplate() throws RenderingException {
        // optimized for performance (via rocker.optimize flag; no auto reloading)
        return new Template(this);
    }

    static public class Template extends com.fizzed.rocker.runtime.DefaultRockerTemplate {

        // \n\n
        static private final byte[] PLAIN_TEXT_0_0;
        // \npackage 
        static private final byte[] PLAIN_TEXT_1_0;
        // \n
        static private final byte[] PLAIN_TEXT_2_0;
        // \n\nimport com.slack.api.app_backend.SlackSignature\nimport io.kotest.core.spec.style.StringSpec\nimport io.kotest.matchers.shouldBe\nimport io.micronaut.context.annotation.Property\nimport io.micronaut.http.HttpRequest\nimport io.micronaut.http.client.HttpClient\nimport io.micronaut.http.client.annotation.Client\nimport io.micronaut.test.extensions.kotest.annotation.MicronautTest\n\nprivate const val TEST_SIGNING_SECRET = \"s3cr3t\"\nprivate val SIGNATURE_GENERATOR = SlackSignature.Generator...
        static private final byte[] PLAIN_TEXT_3_0;
        // &text=hello\"), String::class.java)\n            .body()\n\n        response shouldBe \"{\\\"text\\\":\\\"Got message: Hello\\\"}\"\n    }\n})\n\nprivate fun slackEventRequest(requestBody: String): HttpRequest<String> {\n    val timestamp = \"${System.currentTimeMillis() / 1000}\"\n    val signature = SIGNATURE_GENERATOR.generate(timestamp, requestBody)\n    return HttpRequest.POST(\"/slack/events\", requestBody)\n        .header(SlackSignature.HeaderNames.X_SLACK_REQUEST_TIMESTAMP, timestamp...
        static private final byte[] PLAIN_TEXT_4_0;

        static {
            PlainTextUnloadedClassLoader loader = PlainTextUnloadedClassLoader.tryLoad(commandHandlerTestKotest.class.getClassLoader(), commandHandlerTestKotest.class.getName() + "$PlainText", "UTF-8");
            PLAIN_TEXT_0_0 = loader.tryGet("PLAIN_TEXT_0_0");
            PLAIN_TEXT_1_0 = loader.tryGet("PLAIN_TEXT_1_0");
            PLAIN_TEXT_2_0 = loader.tryGet("PLAIN_TEXT_2_0");
            PLAIN_TEXT_3_0 = loader.tryGet("PLAIN_TEXT_3_0");
            PLAIN_TEXT_4_0 = loader.tryGet("PLAIN_TEXT_4_0");
        }

        // argument @ [3:2]
        protected final Project project;

        public Template(commandHandlerTestKotest model) {
            super(model);
            __internal.setCharset("UTF-8");
            __internal.setContentType(getContentType());
            __internal.setTemplateName(getTemplateName());
            __internal.setTemplatePackageName(getTemplatePackageName());
            this.project = model.project();
        }

        @Override
        protected void __doRender() throws IOException, RenderingException {
            // PlainText @ [5:2]
            __internal.aboutToExecutePosInTemplate(5, 2);
            __internal.writeValue(PLAIN_TEXT_0_0);
            // IfBlockBegin @ [7:1]
            __internal.aboutToExecutePosInTemplate(7, 1);
            if (project.getPackageName() != null) {
                // PlainText @ [7:41]
                __internal.aboutToExecutePosInTemplate(7, 41);
                __internal.writeValue(PLAIN_TEXT_1_0);
                // ValueExpression @ [8:9]
                __internal.aboutToExecutePosInTemplate(8, 9);
                __internal.renderValue(project.getPackageName(), false);
                // PlainText @ [8:34]
                __internal.aboutToExecutePosInTemplate(8, 34);
                __internal.writeValue(PLAIN_TEXT_2_0);
                // IfBlockEnd @ [7:1]
                __internal.aboutToExecutePosInTemplate(7, 1);
            } // if end @ [7:1]
            // PlainText @ [9:2]
            __internal.aboutToExecutePosInTemplate(9, 2);
            __internal.writeValue(PLAIN_TEXT_3_0);
            // ValueExpression @ [32:51]
            __internal.aboutToExecutePosInTemplate(32, 51);
            __internal.renderValue(project.getName(), false);
            // PlainText @ [32:69]
            __internal.aboutToExecutePosInTemplate(32, 69);
            __internal.writeValue(PLAIN_TEXT_4_0);
        }
    }

    private static class PlainText {

        static private final String PLAIN_TEXT_0_0 = "\n\n";
        static private final String PLAIN_TEXT_1_0 = "\npackage ";
        static private final String PLAIN_TEXT_2_0 = "\n";
        static private final String PLAIN_TEXT_3_0 = "\n\nimport com.slack.api.app_backend.SlackSignature\nimport io.kotest.core.spec.style.StringSpec\nimport io.kotest.matchers.shouldBe\nimport io.micronaut.context.annotation.Property\nimport io.micronaut.http.HttpRequest\nimport io.micronaut.http.client.HttpClient\nimport io.micronaut.http.client.annotation.Client\nimport io.micronaut.test.extensions.kotest.annotation.MicronautTest\n\nprivate const val TEST_SIGNING_SECRET = \"s3cr3t\"\nprivate val SIGNATURE_GENERATOR = SlackSignature.Generator(TEST_SIGNING_SECRET)\n\n@MicronautTest\n@Property(name = \"slack.signing-secret\", value = TEST_SIGNING_SECRET)\n@Property(name = \"slack.bot-token\", value = \"\")\nclass CommandHandlerTest(\n    @Client(\"/\") private val client: HttpClient\n) : StringSpec({\n    \"check command handled\" {\n        val response = client\n            .toBlocking()\n            .exchange(slackEventRequest(\"command=/";
        static private final String PLAIN_TEXT_4_0 = "&text=hello\"), String::class.java)\n            .body()\n\n        response shouldBe \"{\\\"text\\\":\\\"Got message: Hello\\\"}\"\n    }\n})\n\nprivate fun slackEventRequest(requestBody: String): HttpRequest<String> {\n    val timestamp = \"${System.currentTimeMillis() / 1000}\"\n    val signature = SIGNATURE_GENERATOR.generate(timestamp, requestBody)\n    return HttpRequest.POST(\"/slack/events\", requestBody)\n        .header(SlackSignature.HeaderNames.X_SLACK_REQUEST_TIMESTAMP, timestamp)\n        .header(SlackSignature.HeaderNames.X_SLACK_SIGNATURE, signature)\n        .header(\"Content-Type\", \"application/json\")\n}\n";

    }

}
